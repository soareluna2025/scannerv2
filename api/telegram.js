...
export default async function handler(req, res) {
  ...
  // GET ?test=1 → sends a test message to verify the integration
  if (req.method === 'GET') {
    if (req.query.test !== '1') return res.status(405).json({ error: 'Method not allowed' });
    ...
    try {
      const r = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: chatId, text: '✅ AlohaScan audit complete — bot funcționează!', parse_mode: 'HTML' })
      });
