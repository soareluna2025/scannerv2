...
> Sursă: audit live (`information_schema` + `pg_indexes` + `pg_stat_user_tables`) + `scripts/create-tables.sql`.
> Ultimă actualizare: 07.06.2026 (rânduri reale din PASUL 1).

---

## ⚠ NOTE DE INTEGRITATE A AUDITULUI

- **Numărul de rânduri** = `n_live_tup` din `pg_stat_user_tables` (PASUL 1, audit 07.06.2026).
...
**Rânduri:** 15.651. **Relații:** team_id→teams, league_id→leagues. Fallback ELO pt meciuri viitoare.
...
**Rânduri:** 189.206. **Relații:** ⋈ fixtures_history, predictions (pe fixture_id). Scris de `build-elo` în replay cronologic.

## `elo_applied`
**Scop:** guard de idempotență — un meci aplicat o singură dată în ELO incremental între reconstrucții.
**Coloane:** `fixture_id` INT **PK**. **Index:** PK(fixture_id). **Rânduri:** 287.236.
...
**Rânduri:** 184.098. **Relații:** ⋈ fixtures_history (fixture_id + team_id↔home/away_team_id).
...
**Rânduri:** 2.258.943. **Relații:** fixture_id→fixtures_history; type='Goal' & detail≠'Own Goal' → HT calc în ML.
...
**Rânduri:** 2.691.740 (cel mai mare tabel). **Relații:** fixture_id→fixtures_history; team_id→teams.
...
**Index:** PK(id); UNIQUE(player_id,league_id,season). **Rânduri:** 99.067.
...
**Index:** PK(id); UNIQUE(team_id,league_id,season). **Rânduri:** 12.237. **Scris de:** collect-daily.
...
**Index:** PK(id); UNIQUE(team_id,league_id,season). **Rânduri:** 1.615.
...
**Index:** PK(id); UNIQUE(league_id,season,team_id). **Rânduri:** 4.772.
...
**Index:** PK(id); UNIQUE(league_id,season,player_id). **Rânduri:** 2.724.
...
**Index:** PK(id); UNIQUE(league_id,season,player_id). **Rânduri:** 2.739.
...
**Index:** PK(league_id). **Rânduri:** 270. **Note:** filtrul real de afișare e în cod (`api/leagues.js` + `league-filter.js`), nu DOAR aici.
...
**Index:** PK(team_id). **Rânduri:** 8.106.
...
**Index:** PK(venue_id). **Rânduri:** 4.604.
...
**Rânduri:** coaches: 11.211 · coach_career: 37.589 · coach_stats: 6.140.
...
**Index:** PK(id); UNIQUE(module,context_key,weight_name). **Rânduri:** 705.
...
**Index:** PK(module,league_group). **Rânduri:** 16. **Grupuri:** low/mid/high (după goluri/meci).
...
**Index:** PK(id); UNIQUE(minute_bucket,score_state,market). **Rânduri:** 175.
...
**Index:** PK(league_id). **Rânduri:** 424.
...
**Index:** PK(id); UNIQUE(fixture_id). **Rânduri:** 1.317.
...
**Index:** PK(fixture_id,stage,data_type); idx_prematch_data_fixture. **Rânduri:** 31.548.
...
**Index:** PK(fixture_id,stage). **Rânduri:** 9.864.
...
**Rânduri:** 811.061 (NGP/OVER15 domină).
...
**Index:** PK(id); idx_cron_logs_job(job_name, ran_at DESC). **Rânduri:** 7.284.
...
**Index:** PK(id); idx_fixture_status_fixture. **Rânduri:** 0.
...
**Index:** PK(league_id). **Rânduri:** 241.
...
**Index:** PK(id); UNIQUE(team1,team2,fixture); idx_h2h_teams(team1_id,team2_id). **Rânduri:** 292.648.
...
**Index:** PK(referee_name). **Rânduri:** 331.
...
**Index:** PK(league_id). **Rânduri:** 651.
...
**Rânduri:** odds: 865.052 · live_odds: 0.
...
**Index:** PK(id); idx_alerts_fixture; idx_alerts_sent. **Rânduri:** 2.460.

## `injuries` / `sidelined`
**Scop:** accidentări per fixture (`injuries`: UNIQUE(fixture_id, player_id)) și indisponibilități per jucător (`sidelined`).
**Rânduri:** injuries: 4 · sidelined: 0.

## `transfers` / `trophies`
**Scop:** transferuri jucători și palmares echipe (metadate UI). **Rânduri:** transfers: 0 · trophies: 0.
...
**Index:** PK(fixture_id). **Rânduri:** 0.

> **Tabele LIVE extra (în DB, dar nu în create-tables.sql):** `app_settings` (PK key) — 642.050 rânduri;
> `cazarma_centrala` (router intern: idx pe sursa/procesat) — 35.520 rânduri.
