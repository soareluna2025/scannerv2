...
# ELO: rebuild COMPLET luni 04:30 (fără query param) + backfill INCREMENTAL zilnic 05:00 (mode=backfill).
30 4 * * 1 curl -sf -H "${HDR}" ${BASE}/api/cron/build-elo >> ${LOG} 2>&1
0 5 * * * curl -sf -H "${HDR}" "${BASE}/api/cron/build-elo?mode=backfill" >> ${LOG} 2>&1
# World Cup — calificări + grupe (standings group-aware), zilnic 05:15.
15 5 * * * curl -sf -H "${HDR}" ${BASE}/api/cron/collect-wc-qualifiers >> ${LOG} 2>&1
