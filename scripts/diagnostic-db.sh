...
h2 "D1. coach_stats — coloane existente"
eval $Q "
...
h2 "F3. fixtures → referee_stats JOIN"
eval $Q "
SELECT
...
h2 "F4. fixtures → teams_stats JOIN"
eval $Q "
SELECT
  COUNT(DISTINCT f.fixture_id)                AS total_fixtures,
  COUNT(DISTINCT f.fixture_id) FILTER
...
h2 "F5. fixtures → form_stats JOIN"
eval $Q "
SELECT
  COUNT(DISTINCT f.fixture_id)                AS total_fixtures,
  COUNT(DISTINCT f.fixture_id) FILTER
...
h2 "G3. Top echipe dupa volum player_stats"
eval $Q "
...
SELECT 'injuries (penalizare)',
  CASE WHEN COUNT(*) > 0 THEN 'OK - ' || COUNT(DISTINCT fixture_id) || ' fixtures'
       ELSE 'GOL - verifica!' END FROM injuries
UNION ALL
SELECT 'prematch predictions API (NECITIT!)',
